import { requireAdminAuth } from '@/src/admin/lib/auth-guard'
import { createServerClient } from '@/src/admin/lib/supabase-server'
import { defaultRealisations } from '@/src/lib/realisations-data'
import Link from 'next/link'
import { Plus } from 'lucide-react'
import RealisationListClient from './RealisationListClient'

export const revalidate = 0

export default async function RealisationsListPage({
  searchParams,
}: {
  searchParams: Promise<{ tab?: string }>
}) {
  await requireAdminAuth()
  const params = await searchParams
  const initialTab = params?.tab === 'trash' ? 'trash' : 'all'

  let realisations: any[] = []

  try {
    const supabase = await createServerClient()
    const { data } = await supabase
      .from('realisations')
      .select('*')
      .order('created_at', { ascending: false })

    if (data && data.length > 0) {
      realisations = data
    } else {
      realisations = defaultRealisations
    }
  } catch (err) {
    console.error('Error fetching realisations:', err)
    realisations = defaultRealisations
  }

  return (
    <div className="space-y-4 w-full">
      {/* Header Card: bg-white and non-bold title */}
      <div className="bg-white border border-[#c3c4c7] rounded-sm p-4 shadow-sm flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 w-full">
        <div>
          <h1 className="text-xl font-normal text-[#1d2327]">Réalisations &amp; Projets de référence</h1>
          <p className="text-xs text-[#646970] font-normal mt-0.5">
            Gérez vos projets réalisés, études de cas et installations industrielles sur site
          </p>
        </div>
        <Link
          href="/admin/realisations/nouveau"
          className="inline-flex items-center gap-1.5 bg-[#2271b1] hover:bg-[#135e96] text-white text-xs font-normal px-3 py-1.5 rounded-sm transition-colors shrink-0"
        >
          <Plus className="w-4 h-4" />
          <span>Nouvelle réalisation</span>
        </Link>
      </div>

      <RealisationListClient initialRealisations={realisations} initialTab={initialTab} />
    </div>
  )
}
