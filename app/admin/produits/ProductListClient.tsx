...
import { Search, Package, Image as ImageIcon, Tag, List, LayoutGrid } from 'lucide-react'
...
export default function ProductListClient({
  ...
  const filteredProducts = products.filter((prod) => {
    const matchesStatus = activeStatus === 'all' || prod.status === activeStatus
    const matchesSearch =
  ...
  return (
    <div className="space-y-4 w-full">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 bg-white border border-[#c3c4c7] rounded-sm p-3 shadow-sm">
        {/* Tabs */}
        <div className="flex items-center gap-2 text-xs border-b border-[#c3c4c7] sm:border-b-0 pb-2 sm:pb-0 select-none">
          ...
          >
            Tous <span className="text-[#646970]">({products.length})</span>
          </button>
          ...
          >
            Publiés <span className="text-[#646970]">({products.filter((p) => p.status === 'published').length})</span>
          </button>
          <span className="text-[#c3c4c7]">|</span>
          <button
            type="button"
          ...
          >
            Brouillons <span className="text-[#646970]">({products.filter((p) => p.status === 'draft').length})</span>
          </button>
        </div>
              ...
              >
                <div>
                  {prod.cover_url ? (
                    ...
                    <div className="h-36 w-full bg-gray-100 rounded-sm border border-gray-200 flex items-center justify-center text-gray-400 mb-3">
