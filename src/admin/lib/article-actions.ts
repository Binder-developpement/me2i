...
// Seed 10 definitive articles
export async function seed10ArticlesAction() {
  const supabase = await createServerClient()

  // Attempt admin login if not already logged in
  ...
  for (const art of articlesToSeed) {
    ...
    if (existing) {
      ...
      results.push({ title: art.title, status: error ? 'error' : 'updated', error: error?.message })
    } else {
      const { data, error } = await supabase
        .from('articles')
        .insert([
      ...
      results.push({ title: art.title, status: error ? 'error' : 'inserted', error: error?.message })
    }
  }
