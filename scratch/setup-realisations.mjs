...
const supabaseUrl = 'https://aivmdijdocgbnuxqwsbe.supabase.co'
const supabaseKey = 'sb_publishable_qxSwoOdFqenK5dPFYEcUAA_NRN5gOe0'
...
const initialRealisations = [
  ...
  },
  {
    ...
    content: "Conception sur mesure et fabrication d'un coffret inverseur de source automatique (ATS) pour une boulangerie industrielle. Ce coffret chemically et physiquement sécurisé assure la bascule automatique du réseau vers le groupe électrogène en moins de 10 secondes en cas de coupure de courant, évitant ainsi l'interruption de la chaîne de cuisson et la perte de marchandise. Il intègre un verrouillage mécanique et électrique entre les contacteurs pour empêcher tout risque de retour de tension.",
  ...
  },
  {
    ...
    description: "Intervention de maintenance corrective sur un groupe électrogène industriel sur site. Les techniciens ME2I procèdent au diagnostic et à la réparation des composants défaillants directement chez le client : contrôle du circuit de démarrage, vérification des connexions électriques, remplacement des pièces usées et tests de mise en charge.",
    content: "Intervention de maintenance corrective sur un groupe électrogène industriel sur site. Les techniciens ME2I procèdent au diagnostic et à la réparation des composants défaillants directement chez le client : contrôle du circuit de démarrage, vérification des connexions électriques, remplacement des pièces usées et tests de mise en charge.",
...
async function seedRealisations() {
  console.log('Seeding realisations table into Supabase...')
  const { data, error } = await supabase.from('realisations').upsert(initialRealisations, { onConflict: 'id' })

  if (error) {
    console.error('Error inserting realisations:', error.message)
  } else {
    console.log(`Successfully seeded ${initialRealisations.length} realisations into Supabase!`)
  }
}
